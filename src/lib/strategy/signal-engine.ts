...
export function generateSignal(indicators: Indicators, trend: TrendDirection = 'SIDEWAYS'): Signal {
  ...
  const bullish = ema3 > ema8 && currentPrice > ema50 && volatilityRatio >= 0.4 && adx >= 22 && rsi < 70 && plusDi > minusDi
  const bearish = ema3 < ema8 && currentPrice < ema50 && volatilityRatio >= 0.4 && adx >= 22 && rsi > 30 && minusDi > plusDi

  if (bullish && trend === 'UP') {
    action = 'BUY'
    confidence = Math.min(1, emaGap * 5 + adx / 100)
    reason = `Bullish: EMA3>EMA8, price>EMA50, trend UP, ADX=${adx.toFixed(1)}, +DI>−DI`
  } else if (bearish && trend === 'DOWN') {
    action = 'SELL'
    confidence = Math.min(1, emaGap * 5 + adx / 100)
    reason = `Bearish: EMA3<EMA8, price<EMA50, trend DOWN, ADX=${adx.toFixed(1)}, −DI>+DI`
  } else {
